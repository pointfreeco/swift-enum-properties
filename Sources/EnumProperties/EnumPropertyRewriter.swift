...
public class EnumPropertyRewriter: SyntaxRewriter {
  public init(includePlainEnums: Bool) {
    self.includePlainEnums = includePlainEnums
  }
  let includePlainEnums: Bool
  var indents: [Int] = []
  ...
  override public func visit(_ node: EnumDeclSyntax) -> DeclSyntax {
    ...
    guard
      !enumCases.isEmpty,
      includePlainEnums || allEnumCases.contains(where: { $0.associatedValue != nil })
      else { return node }
